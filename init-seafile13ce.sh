...
# ==================== 首次部署配置区 ====================
# 首次运行：环境变量 > 下方默认值。已有部署：只读取部署目录 .env。
# 协议/主机为用户访问地址；外部反代模式请填公网域名，内层仍使用 HTTP。
...
# 空值自动生成；外部反代可填 http://域名, http://内网IP（逗号后空格）。
CADDY_SITE="${CADDY_SITE:-}"
# 只信任实际反代来源 IP/CIDR；Docker NAT 后的来源可能不是 127.0.0.1。
...
IMAGE_PREFIX="${IMAGE_PREFIX:-}"  # 镜像代理主机，可带端口，不带协议。
CONTAINER_PREFIX="${CONTAINER_PREFIX:-seafile13ce}"
DOCKER_COMMAND="${DOCKER_COMMAND:-}" # 自动选择 pg-docker 或 docker
DOCKER_COMPOSE_COMMAND="${DOCKER_COMPOSE_COMMAND:-}" # 空值使用上面命令 + compose
DOCKER_SOCKET="${DOCKER_SOCKET:-}" # 必须是同一 daemon 在服务端的 socket 路径
SEAFILE_IMAGE="${SEAFILE_IMAGE:-seafileltd/seafile-mc:13.0-latest}"
...
THUMBNAIL_SERVER_IMAGE="${THUMBNAIL_SERVER_IMAGE:-seafileltd/thumbnail-server:13.0-latest}"
...
if [[ ! -f "$TARGET/.env" ]]; then
  [[ ! -d "$TARGET/data" ]] || fail '存在 data 但缺少 .env；请先恢复原配置，禁止重置凭据'
  [[ ! -f "$TARGET/docker-compose.yml" ]] || fail '已有未知 Compose 配置，请选择空目录'
...
# COMMON_END
load_env "$ENV_INPUT"
if [[ "$ENV_INPUT" == "$TARGET/.env.new" ]]; then
  mv "$ENV_INPUT" "$TARGET/.env"
else
  BACKUP="$TARGET/config-backups/$(date +%Y%m%d-%H%M%S)-$$"
  mkdir -p "$BACKUP"
  for file in .env docker-compose.yml common.sh configure.py verify.py verify.sh deploy.sh compose.sh README.txt; do
...
trap 'echo "部署/验收失败。请运行 ./compose.sh ps 和 ./compose.sh logs；修复后重跑 ./deploy.sh。" >&2' ERR
...
./deploy.sh 部署或修复；./verify.sh 业务验收；./compose.sh ps / logs 查看状态。
PULL=1 ./deploy.sh 才主动刷新已有镜像；缺少镜像时 Compose 自动拉取。
修改 .env 后重新运行单脚本，以更新生成文件；已有 .env 优先于环境变量和默认值。
请勿修改项目名/容器前缀后直接复用运行中的目录；不要将 Pro 数据直接降级到 CE。
所有配置和数据均存储在本目录。config-backups 仅备份配置，不能替代数据库和文件备份。
